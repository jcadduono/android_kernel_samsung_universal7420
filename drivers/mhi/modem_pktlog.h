...
#define pktlog_tx_bottom_skb(pktlog, skb)	pktlog_queue_skb(pktlog, (PKT_TX | PKT_BOT) , skb)
#define pktlog_tx_top_skb(pktlog, skb)	pktlog_queue_skb(pktlog, (PKT_TX | PKT_TOP) , skb)
#define pktlog_rx_bottom_skb(pktlog, skb)	pktlog_queue_skb(pktlog, (PKT_RX | PKT_BOT) , skb)
#define pktlog_rx_top_skb(pktlog, skb)	pktlog_queue_skb(pktlog, (PKT_RX | PKT_TOP) , skb)
#define pktlog_text_buf(pktlog, buf)	do {} while(0)

#else /* else of DEBUG_PKTLOG */
inline static struct pktlog_data *create_pktlog(char *name) { return NULL; }
#define remove_pktlog(pktlog)		do {} while(0)
#define pktlog_tx_bottom_skb(pktlog, skb)	do {} while(0)
#define pktlog_tx_top_skb(pktlog, skb)	do {} while(0)
#define pktlog_rx_bottom_skb(pktlog, skb)	do {} while(0)
#define pktlog_rx_top_skb(pktlog, skb)	do {} while(0)
#define pktlog_text_buf(pktlog, buf)	do {} while(0)
