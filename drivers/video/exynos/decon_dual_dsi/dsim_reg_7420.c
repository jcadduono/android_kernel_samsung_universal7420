...
int dsim_reg_enable_pll(u32 id, u32 en)
 {
	...
	} else if (id == 1) { /* DSIM1 uses external PLL which is DPHY PLL in CMU */
		if (en) {
			dsim_write_mask(id, DSIM_PHYCTRL_M2, ~0, DSIM_PHYCTRL_M2_SEL_HS_CLK_SRC);
			decon_clk_set_rate(dsim1->dev, "dphy_pll", 1026 * MHZ);
			val = readl(dsim1->reg_dphy_pll_con + DPHY_PLL_CON1);
			val |= DPHY_PLL_CON1_LOCK_EN;
			writel(val, dsim1->reg_dphy_pll_con + DPHY_PLL_CON1);

			while (1) {
				cnt--;
				val = readl(dsim1->reg_dphy_pll_con);
				if (val & DPHY_PLL_CON0_LOCKED)
					return 0;
				if (cnt == 0)
					return -EBUSY;
			}
			...
			/* TODO: disable DPHY PLL or set low clock */
 		}
#endif
	} else {
		dsim_err("not supported DSIM id(%d)\n", id);
		return -EINVAL;
 	}
...
void dsim_reg_write_pkt_hdr(u32 id, u32 pkthdr)
{
	dsim_write(id, DSIM_PKTHDR, pkthdr);
}

void dsim_reg_wr_tx_header(u32 id, u8 cmd, u8 d0, u8 d1)
{
	u32 val = 0;

	val |= (u32)(DSIM_PKTHDR_ID(cmd) | DSIM_PKTHDR_DATA0(d0) | DSIM_PKTHDR_DATA1(d1));
...
int dsim_reg_wait_enter_ulps_state(u32 id, u32 lanes)
{
	...
	if (!cnt) {
		dsim_err("DSI%d Master is not ULPS state.\n", id);
		return -EBUSY;
	}
