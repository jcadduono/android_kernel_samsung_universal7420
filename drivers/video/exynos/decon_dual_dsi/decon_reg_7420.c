...
void decon_reg_init(u32 id, enum decon_dsi_mode dsi_mode, struct decon_init_param *p)
{
	...
	if (dsi_mode == DSI_MODE_DUAL)
	{
		...
		decon_write_mask(id, FRAMEFIFO_REG9, ~0, FRAMEFIFO_FIFO_SHARE_EN);
		/* TODO: if MIC is enabled ? */
		decon_write_mask(id, FRAMEFIFO_REG10, FRAMEFIFO_SPLIT_IDX_F(lcd_info->xres),
				FRAMEFIFO_SPLIT_IDX_F_MASK);
		decon_info("%s: FRAMEFIFO_REG9(0x%x), FRAMEFIFO_REG10(0x%x)\n", __func__,
				decon_read(id, FRAMEFIFO_REG9), decon_read(id, FRAMEFIFO_REG10));
	}
